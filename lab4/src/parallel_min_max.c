...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < pnum; i++) {
    ...
    if (child_pid >= 0) {
      ...
      if (child_pid == 0) {
        if (timeout_expired) {
          printf("Child proccess terminated due to timeout\n");
          exit(0);
        }
        // child process
        ...
        // parallel somehow
        if (i != pnum - 1) {
          min_max = GetMinMax(array, i * m, (i + 1) * m);
        }
        else {
          min_max = GetMinMax(array, i * m, array_size);
        }
        if (with_files) {
          fprintf(file_out, "%d %d\n", min_max.min, min_max.max);
        } else {
          write(fd[i][1], &min_max.min, sizeof(int));
          write(fd[i][1], &min_max.max, sizeof(int));
          close(fd[i][0]);
          close(fd[i][1]);
        }
        return 0;
  ...
  while (active_child_processes > 0) {
    if (timeout_expired) {
      printf("Killing child proccesses due to timeout.\n");
      for (int i = 0; i < pnum; i++) {
        if (child_pids[i] > 0) {
    ...
    int status;
    pid_t state = waitpid(-1, &status, WNOHANG);
    if (state > 0)
      active_child_processes -= 1;
  }
